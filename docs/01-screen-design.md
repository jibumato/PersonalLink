...
**E2EEはやるかどうかではなく、いつやるかの判断。** 憲法第十一条「企業を信じなくてよい」を満たす手段はE2EEしかない([D-17](#d-17-サーバーに保存しないではなくサーバーが読めないを目指す) / [docs/09-data-residency.md](./09-data-residency.md))。MVPで見送るのはマルチデバイス鍵同期の複雑さゆえで、Phase 3(Universal Identity 基盤)と同時に導入する。それまでは通信路暗号化+保存時暗号化とし、**満たせていないことを仕様書に明記して正直に運用する**。データモデルはE2EE移行を妨げない形にしておく。
...
### D-17: 「サーバーに保存しない」ではなく「サーバーが読めない」を目指す

保存期間は Connection の寿命に縛る。完全P2Pは対面セッションに限定して検討する。

**理由**: 「保存していません」は**ユーザーから検証できない**ので、結局「企業を信じてください」に
帰着してしまう(第十一条が唯一否定していること)。また端末にデータを閉じ込める構成は、
機種変更で消える(第三条違反)・持ち出す先がない(第六条違反)。
憲法が要求しているのは非保存ではなく **E2EE** である。

**いますぐやること**: Connection 終了時に本文を物理削除し、「**保存期間 = 関係の寿命**」を仕様にする。
関係に終わりがあるプロダクトにしか書けない仕様で、実装は既存の `ON DELETE CASCADE` で足りる。

根拠と4方式の比較、通報の成立条件、段階案は **[docs/09-data-residency.md](./09-data-residency.md)** に置いた。
...
| 0.1.6 | 2026-09 | **D-17(サーバーが読めないことを目指す)を追加**。「メッセージをサーバーに保存しない構成」を検討した結果、完全P2P・非保存リレーはいずれも憲法第三条・第六条・第十一条に反すると判断した。あわせて **D-9 の位置づけを「難しいから後回し」から「第十一条を満たす唯一の手段。時期の判断」へ改めた**。根拠は [09-data-residency.md](./09-data-residency.md) |
