...
### D-7: 電話番号・メールアドレスを一切収集しない

登録は `@ID + Passkey` のみ。アカウント回復は**リカバリーコード**(登録時に1回だけ表示・保存必須)と**複数端末へのPasskey追加**で行う(憲法第二条: 最小収集)。
...
```mermaid
flowchart TD
    A1[A-1 ウェルカム] --> A2[A-2 ID取得] --> A3[A-3 Passkey] --> A4[A-4 プロフィール] --> B1[B-1 ホーム]
    B1 <--> B2[B-2 マイQR表示]
    B1 <--> B3[B-3 QR読み取り]
...
| あなた \ 相手 | 継続 | 継続しない | 未選択 |
|---|---|---|---|
| **継続** | ♾ 恒久化(そろった瞬間に即時) | 期限で終了 | 期限+猶予で終了 |
| **継続しない** | 期限で終了 | 期限で終了 | 期限で終了 |
| **未選択** | 期限+猶予で終了 | 期限で終了 | 期限で終了 |

重要な不変条件(D-3):

- 「継続しない」が選択されても、**終了は自然な期限時刻まで実行しない**(早期終了すると相手に拒否がバレるため)。
...
- サブコピー「電話番号もメールも不要。期限付きで、気軽につながる。」
- ボタン: [はじめる](主)/ [ログイン](副)
- **QR読み取り経由の場合**: 「`@tanaka` さんとつながるためにアカウントを作成します」と相手のアバター・表示名を表示(接続チケットを保持したまま登録フローへ。§B-3参照)

**アクションと遷移**
- [はじめる] → A-2
- [ログイン] → Passkey認証(WebAuthn)→ B-1。失敗時: リカバリーコードによる回復導線

**状態・エッジケース**
- ログイン済みでアクセス → B-1へリダイレクト
- Passkey非対応ブラウザ → 「対応ブラウザ(iOS Safari 16+ / Android Chrome 108+ 等)でお使いください」の案内。**MVPはPasskey必須**(D-7、パスワードは作らない)
...
**目的**: パスワードレス認証の確立とアカウント回復手段の確保。

**表示要素**
- 説明: 「パスワードは作りません。この端末の顔認証・指紋がそのまま鍵になります」
- [Passkeyを作成] → WebAuthn(platform authenticator, resident key)
- 成功後: **リカバリーコード**(例: 8ブロック×4文字)を1回だけ表示
  - [コピー] / [保存した] チェックボックス(チェックまで次へ進めない)
  - 警告: 「電話番号もメールも登録しないため、このコードだけがアカウント回復の手段です」

**アクションと遷移**
- Passkey作成成功 + 保存確認 → A-4

**状態・エッジケース**
- WebAuthn失敗(キャンセル等)→ リトライ導線
- 回復フロー(ログイン画面から): `@ID` + リカバリーコード → 新Passkey登録 → **全端末ログアウト** + コード再発行

**計測**: `passkey_created` / `recovery_code_saved`
...
- Passkey一覧: 登録先端末 / 作成日 / [削除]
- 「複数の端末にPasskeyを追加しておくと、端末を失くしても安心です」+ [この端末以外にPasskeyを追加]
- [**全端末からログアウト**](MVP必須機能⑩。現在の端末を残すか選択可)

**計測**: `logout_all_executed` / `passkey_added {count}`
...
1. **技術選定メモの作成**(次セッション): SPA + API構成。候補: Next.js(App Router)+ PostgreSQL + WebSocket(or SSE)。WebAuthn は SimpleWebAuthn 等の実績あるライブラリを使用
2. **スプリント分割案**:
   - S1: 認証基盤(A-1〜A-4、F-3)— Universal ID + Passkey + セッション管理
