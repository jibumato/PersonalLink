...
**▶ 次のアクション:**
1. 👤 **オーナー: 認証方式の決定**([D-7](docs/01-screen-design.md) / [T-4](docs/05-tech-stack.md))— S1 の本人確認実装のブロッカー
2. 🤖 方式決定を待たずに **S1 の前半**(DBスキーマ・@ID取得・DBセッション)へ着手可能
3. 👤 Vercel / Neon アカウント(S1 中に必要)
...
| M2 | S1 認証基盤 | 08/20〜09/02 | ⬜ | 登録→ログアウト→再ログイン→回復 が通る | アカウントが持てる(**認証方式の決定待ち**) |
...
| タスク | 担当 | Status |
|---|---|---|
**前半 — 認証方式の決定を待たずに着手できる**

| タスク | 担当 | Status |
|---|---|---|
| DBスキーマ(users / sessions / profiles)+ Drizzleマイグレーション | 🤖 | ⬜ |
| @ID取得(重複チェック・予約語・90日ルール)+ A-2 IME対応 | 🤖 | ⬜ |
| **DBセッション**(発行・検証・revoke)— 方式非依存(T-4) | 🤖 | ⬜ |
...
> 💡 決定が遅れる場合は、前半だけ実装して**開発用の仮ログイン**(@ID を入れるだけ)を置けば、
> **S2以降(QR接続・チャット・期限エンジン)は先に進められる**。セッション層が方式非依存なので、
> 後から本物の本人確認を差し込むだけで済む。
