...
**Phase 1(Web版MVP)— 仕様策定フェーズ完了。実装フェーズ開始前。**
...
| M1 | S0 技術選定・環境構築 | 08/13〜08/19 | ⬜ | 本番URLでhello world+CI green | 開発が回り始める |
...
| タスク | 担当 | Status |
|---|---|---|
| 技術選定メモ作成(`docs/05-tech-stack.md`)— 候補: Next.js + PostgreSQL + SSE/WS + SimpleWebAuthn | 🤖 | ⬜ |
| **WebAuthnスパイク: iOS Safari / Android Chrome 実機でPasskey作成・認証を検証**(最重要リスクの先行潰し) | 🤖👤 | ⬜ |
| リポジトリ構成(app/ 作成、lint・型チェック・テストのCI) | 🤖 | ⬜ |
| デプロイ先決定(Vercel等)+ 本番/プレビュー環境 | 👤🤖 | ⬜ |
| DB(Postgres)とストレージ(写真用)のホスティング決定 | 👤🤖 | ⬜ |
...
| サービス名の商標・ドメイン確認(「PersonalLink」続投可否) | 👤 | S3完了まで | ⬜ |
...
| リスク | 影響 | 対策 | 状態 |
|---|---|---|---|
| WebAuthn実機互換(特にiOS Safari) | S1が崩れる=登録体験全体 | **S0でスパイク検証を最優先**。だめなら登録フロー再設計 | ⬜ 未検証 |
