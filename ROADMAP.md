...
**Phase 1(Web版MVP)— S5 完了。コアループに加えてレベル・安全機能・データ主権がそろい、人に渡せる状態になった。次は S6(グループ・計測)。**
...
**▶ 次のアクション:**
1. 🤖 **S6(グループ・計測)へ着手可能** — ここで機能が出そろう(β候補)
...
| Phase | 内容 | 状態 | 時期(目安) |
|---|---|---|---|
| **Phase 1** | **Web版MVP** | 🔄 S5まで実装済み(S1後半は方式待ち) | **2026 Q3〜Q4** |
...
| βイベントの候補リストアップ | 👤 | S6完了まで | ⬜(S5完了により**着手時期に入った**)|
| デプロイ時の環境変数: `CRON_SECRET`(期限ジョブの保護)/ `PL_DEV_LOGIN` を**本番で設定しない** | 👤 | デプロイ時 | ⬜ **S4で追加** |
